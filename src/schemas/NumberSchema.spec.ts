...
describe("NumberSchema", () => {
  ...
  test("optional", async () => {
    ...
    const errors1 = (await s.verifyAsync("1"))!
  ...
  test("min", async () => {
    ...
    expect(await s1.verifyAsync(min)).toBe(undefined)
  ...
  test("max", async () => {
    ...
    expect((await s1.verifyAsync(4))![0].message).toBe(
      translateMessage("number_max", [max])
    )
    expect(await s1.verifyAsync(max)).toBe(undefined)
  ...
  test("between", async () => {
    ...
    expect(await s1.verifyAsync(3)).toBe(undefined)
  ...
  test("integer", async () => {
    ...
    expect((await s.verifyAsync(1.2))![0].message).toBe(
